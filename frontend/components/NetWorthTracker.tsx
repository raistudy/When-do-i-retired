...
async function exportPDF(snap: any, currency: string) {
  ...
  const row = (label: string, value: string, sub?: string) => {
    ...
    y += lh - 1;
    if (sub) { doc.setFontSize(9); doc.setTextColor(120); doc.text(sub, indent + 4, y); doc.setFontSize(10); doc.setTextColor(0); y += lh - 2; }
  };
  const divider = () => { doc.setDrawColor(200); doc.line(indent, y, 195, y); y += 4; };
...
function HistoryRow({ snap, prev, currency, isFirst }: { snap: any; prev: any; currency: string; isFirst: boolean }) {
  ...
  return (
    <div style={{ background: C.cream, border: `2px solid ${C.ink}`, borderRadius: 14, marginBottom: 8, overflow: "hidden", boxShadow: `2px 2px 0 ${C.ink}`, fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      {/* Row header */}
      <div onClick={() => setOpen(!open)} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 90px 90px", alignItems: "center", gap: 8, padding: "12px 14px", cursor: "pointer" }}
        ...
        <div style={{ fontWeight: 800, fontSize: 16 }}>{fmtShort(nw, currency)}</div>
        <div style={{ fontWeight: 800, fontSize: 13, color: delta == null ? "#888" : delta >= 0 ? C.teal : C.coral }}>
          {delta == null ? "—" : `${delta >= 0 ? "+" : ""}${fmtShort(delta, currency)}`}
        </div>
        <button style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, fontWeight: 700, padding: "5px 10px", border: `2px solid ${C.ink}`, borderRadius: 8, background: C.cream, cursor: "pointer", whiteSpace: "nowrap" }}>
          {open ? "Close ▴" : "Details ▾"}
        </button>
      </div>
  ...
  return (
    <main style={{ maxWidth: 680, margin: "0 auto", padding: "2rem 1.2rem", fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        {tabs.map(tb => (
          <button key={tb.key} onClick={() => setTab(tb.key)} style={{
            background: tab === tb.key ? C.ink : C.paper, color: tab === tb.key ? C.paper : C.ink,
            border: `2px solid ${C.ink}`, borderRadius: 10, padding: "7px 14px", fontWeight: 700, fontSize: 13, cursor: "pointer",
            fontFamily: "'IBM Plex Mono', monospace",
          }}>{tb.label}</button>
        ))}
      </div>
      ...
      {tab === "dashboard" && <>
        ...
        <Card title={t.pillar3_title}>
          {essentialTotal <= 0
            ...
            : <>{t.pillar3_emergency} <b>{fmt(emergencyFund, currency)}</b><br />{t.pillar3_essential} <b>{fmt(essentialTotal, currency)}/mo</b><br />{t.pillar3_runway} <b>{runway.toFixed(1)} {t.pillar3_months}</b></>
          }
        </Card>
        {result && (
        ...
        {history.length > 0 && (
          <div style={{ marginTop: 28, borderTop: `2px dashed rgba(27,27,27,0.15)`, paddingTop: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 90px 90px", gap: 8, padding: "4px 14px", fontSize: 10, opacity: 0.5, fontWeight: 700, marginBottom: 4, fontFamily: "'IBM Plex Mono', monospace" }}>
              <span>DATE</span><span>NET WORTH</span><span>CHANGE</span><span></span>
            </div>
            {history.map((snap, i) => (
              <HistoryRow key={snap.id} snap={snap} prev={history[i + 1] ?? null} currency={currency} isFirst={i === 0} />
            ))}
      ...
      {tab === "save" && <>
        ...
        <p style={{ fontSize: 13, opacity: 0.55, marginBottom: 10 }}>{user ? t.save_subtitle_user : t.save_subtitle_guest}</p>
        ...
        {saveMsg && (
          <div style={{ fontSize: 13, marginBottom: 12, padding: "8px 14px", background: C.cream, border: `2px solid ${C.ink}`, borderRadius: 12, boxShadow: `2px 2px 0 ${C.ink}` }}>{saveMsg}</div>
        )}
        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          <Btn onClick={onBack}>{t.back_home}</Btn>
          <Btn primary onClick={calculate} disabled={loading}>{loading ? t.saving : t.calculate_save_btn}</Btn>
        </div>
      </>}
